        ...
        for chan in chans {
            match chan {
                Channel::Orderbooks { xch, pair } => {
                    ...
                    let records = read_order_books_df(partitions, self.sample_rate).await?;
...
async fn read_order_books_df(partitions: Vec<String>, sample_rate: Duration) -> Result<Vec<RecordBatch>> {
    let mut ctx = ExecutionContext::new();
    dbg!(&partitions);
    let mut records = vec![];
    for partition in partitions {
        ctx.register_avro("order_books", &partition, AvroReadOptions::default())?;
        let sql_query = format!("select event_ms, asks[0][0] as a1, asks[0][1] as aq1, asks[1][0] as a2, asks[1][1] as aq2, asks[2][0] as a3, asks[2][1] as aq3, asks[3][0] as a4, asks[3][1] as aq4, asks[4][0] as a5, asks[4][1] as aq5, bids[0][0] as b1, bids[0][1] as bq1, bids[1][0] as b2, bids[1][1] as bq2, bids[2][0] as b3, bids[2][1] as bq3, bids[3][0] as b4, bids[3][1] as bq4, bids[4][0] as b5, bids[4][1] as bq5 from
   (select asks, bids, event_ms, ROW_NUMBER() OVER (PARTITION BY sample_time order by event_ms) as row_num
    FROM (select asks, bids, event_ms / {sample_rate} as sample_time, event_ms from order_books)) where row_num = 1;", sample_rate = sample_rate.num_milliseconds());
