...
impl BreakoutStrategy {
    pub fn new(name: String, options: &BreakoutStrategyOptions) -> Self {
        Self {
            ...
            ph: UpperReversalSignal::new(options.lb_high() as u8, options.lb_high() as u8, &0.0).unwrap(),
            pl: LowerReversalSignal::new(options.lb_low() as u8, options.lb_low() as u8, &0.0).unwrap(),
            ...
            ma_crossed_at: Utc.timestamp_millis(0),
            last_stop_level: f64::NAN,
            last_close: f64::NAN,
            last_stop_define: f64::NAN,
            last_buy_level: f64::NAN,
            lows: Window::new(options.lb_low() as u8, f64::NAN),
            highs: Window::new(options.lb_high() as u8, f64::NAN),
            high_level: f64::NAN,
            low_level: f64::NAN,
        }
    }
...
impl Strategy for BreakoutStrategy {
    ...
    async fn eval(
        ...
        let trail_cross = last_pos.map_or(Utc.timestamp_millis(0), |pos| pos.meta.open_at) < self.ma_crossed_at;
        if is_long {
