...
use broker_core::types::OrderType;

#[derive(Debug)]
pub(crate) struct BinanceFeeProvider {
    bnb_burn: bool,
    #[allow(dead_code)]
    vip_level: i8,
}
...
impl BinanceFeeProvider {
    #[allow(dead_code)]
    pub(crate) fn new(bnb_burn: bool, vip_level: i8) -> Self { Self { bnb_burn, vip_level } }
}
...
impl FeeProvider for BinanceFeeProvider {
    fn get_rate(&self, asset_type: Option<AssetType>, order_type: Option<OrderType>) -> Fee {
        ...
        let fee_rate = match asset_type.unwrap_or(AssetType::Spot) {
