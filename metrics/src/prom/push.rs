...
pub fn make_client() -> Client {
    let conn = awc::Connector::new()
        ...
        .limit(10);
    Client::builder().connector(conn).timeout(CLIENT_TIMEOUT_SEC).finish()
}
...
pub async fn push_metrics<S: BuildHasher>(
    ...
    push(client, job, grouping, url, mfs, "PUT", basic_auth).await
}
...
pub async fn push_add_metrics<S: BuildHasher>(
    ...
    push(client, job, grouping, url, mfs, "POST", basic_auth).await
}
...
async fn push<S: BuildHasher>(
    ...
    if job.contains('/') {
        return Err(Error::Msg(format!("job contains '/': {}", job)).into());
    }
...
//         async {
//             registry.register(bc).map_err(|e| std::io::Error::new(Error::Other, e))
//         }.await?
//     }
//
//     let mfs = registry.gather();
//     push(client, job, grouping, url, mfs, method, basic_auth).await
// }
//
...
//     basic_auth: Option<BasicAuthentication>,
// ) -> Result<()> {
//     push_from_collector(client, job, grouping, url, collectors, "PUT", basic_auth).await
// }
//
...
//     basic_auth: Option<BasicAuthentication>,
// ) -> Result<()> {
//     push_from_collector(client, job, grouping, url, collectors, "POST", basic_auth).await
// }
...
mod tests {
    ...
    async fn test_push_bad_label_name() {
        ...
        for case in table {
            ...
            let label_vec: RepeatedField<proto::LabelPair> = vec![l].into();
            m.set_label(label_vec);
            let mut mf = proto::MetricFamily::new();
            mf.set_metric(vec![m].into());
            let res = push_metrics(&client, "test", hostname_grouping_key(), "mockurl", vec![mf], None).await;
