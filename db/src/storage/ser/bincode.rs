...
impl<T: Storage + ?Sized> BincodeStorageExt for T {
    fn put<K, V>(&self, table: &str, key: K, value: V) -> Result<()>
    where
        K: AsRef<[u8]>,
        V: Serialize,
    {
        let serialized = bincode::serialize::<V>(&value)?;
        ...
        let vec: Vec<BatchOperation> = values
            ...
            .map(|(t, k, v)| {
                (
                    *t,
                    k.as_ref(),
                    v.as_ref().map(|v| bincode::serialize(v.as_ref()).unwrap()),
                )
            })
            .collect();
        self._batch(&vec)
        ...
        V: DeserializeOwned,
    {
        ...
        Ok(bincode::deserialize(record.as_slice())?)
    }
        ...
        V: DeserializeOwned,
    {
        ...
        items.iter().map(|v| bincode::deserialize(v).err_into()).collect()
    }
        ...
        V: DeserializeOwned,
    {
        ...
        items
            .into_iter()
            .map(|(k, v)| bincode::deserialize(&v).err_into().map(|d| (k, d)))
            .collect()
    }
        ...
        V: DeserializeOwned,
    {
        ...
        items
            .into_iter()
            .map(|(k, v)| bincode::deserialize::<V>(&v).map(|v| (k, v)).err_into())
            .collect()
    }
