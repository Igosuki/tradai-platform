use crate::summary::data::DataSummary;
...
impl PositionSummariser for PnLReturnSummary {
    fn update(&mut self, position: &Position) {
        ...
        if self.total.count == 0 {
            self.start_timestamp = position.meta.open_at;
        }
...
impl PnLReturnSummary {
    ...
    fn update_trading_session_duration(&mut self, position: &Position) {
        self.duration = match position.meta.close_at {
            None => {
                // Since Position is not exited, estimate duration w/ last_update_timestamp
                position.meta.last_update.signed_duration_since(self.start_timestamp)
            }
...
mod tests {
    ...
    fn update_trading_session_duration_with_non_exited_position() {
        ...
        input_position.meta.close_at = None;
        input_position.meta.last_update = base_timestamp.checked_add_signed(Duration::days(10)).unwrap();
    ...
    fn update_trading_session_duration_with_exited_position() {
        ...
        let mut input_position = Position::default();
        input_position.meta.close_at = Some(base_timestamp.checked_add_signed(Duration::days(15)).unwrap());
