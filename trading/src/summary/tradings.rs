use crate::indicators::ratio::{CalmarRatio, Ratio, SharpeRatio, SortinoRatio};
use crate::summary::drawdown::DrawdownSummary;
...
pub fn trading_duration(start_timestamp: &DateTime<Utc>, position: &Position) -> Duration {
    match position.meta.close_at {
        None => {
            // Since Position is not exited, estimate duration w/ last_update_timestamp
            position.meta.last_update.signed_duration_since(*start_timestamp)
        }
